...
export class KnowledgeRepository {
  ...
  // namespace 省略 = 跨分区取同名 doc(仅后台单库视图用);多租户务必传 namespace。
  // limit 供管理预览截断;不传时保留内部 freshness 校验所需的完整结果。
  kbChunksByDoc(
    doc: string,
    namespace?: string,
    limit?: number
    ...
    const where =
      namespace === undefined ? "WHERE doc = ?" : "WHERE doc = ? AND namespace = ?"
    const params: (string | number)[] =
      namespace === undefined ? [doc] : [doc, namespace]
    const tail = limit === undefined ? "" : " LIMIT ?"
    if (limit !== undefined) params.push(limit)
    return this.sql
      .prepare<{ id: number; content: string }>(
        `SELECT id, content FROM kb_chunks ${where} ORDER BY id${tail}`
      )
      .all(...params)
  }
