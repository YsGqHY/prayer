...
// 返回 false 让手动入口能报告失败(任一分区失败即为 false);定时入口仍按既有周期推进游标。
export async function runCompact(
  ...
  for (const [namespace, entries] of byNamespace) {
    ...
    if (!(await compactNamespace(d, namespace, entries))) ok = false
  }
  return ok
}

// 单分区整理:超过 batchSize 时分批调 LLM,各批结果汇总后一次性替换该分区;
// 单批失败则该批保留原文,其它批仍生效。返回 false 表示该分区有批次失败或提交落空。
async function compactNamespace(
  ...
  try {
    ...
    logger.log(
      "info",
      `[reflection-compact] [${namespace}] ${entries.length} → ${allFaqs.length} 条${batchNote}${chunkNote}`
    )
    ...
    if (d.notifyAdmin && d.adminSurface) {
      bus.emit("action.send", {
        ...
        text: `反思整理[${namespace}]:${entries.length} → ${allFaqs.length} 条${batchNote}${chunkNote}`,
      })
    }
